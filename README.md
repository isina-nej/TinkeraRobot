...
- **نسخه پرامپت**: `NOYA_SYSTEM_PROMPT_VERSION = "v2"`
...
- **هویت سازنده:** با `NOYA_CREATOR_IDS` / `NOYA_CREATOR_NAME` (پیش‌فرض نام «ادی»، در صورت خالی بودن آیدی‌ها از `ADMIN_IDS`) نویا می‌فهمد سازنده‌اش کیست؛ هنگام چت، بلوک `[SPEAKER]` با `role=creator|user` به پیام اضافه می‌شود.
- **ویژگی‌ها:**
