...
def _strip_noya_prefix(text: str) -> str | None:
...
class AgentTriggerFilter(BaseFilter):
    """Match "نویا، <admin command>" only for admins with a recognised intent."""
    ...
    async def __call__(self, message: Message, bot) -> bool | dict:
        ...
        if not message.text or message.chat.type not in {ChatType.GROUP, ChatType.SUPERGROUP}:
            return False
        command = _strip_noya_prefix(message.text)
        if not command:
            return False
        # Cheap check first: only proceed if this parses to an admin intent.
        if deterministic_parse(command) is None:
            return False
        # Only then verify admin status (kept out of the hot path for chat).
