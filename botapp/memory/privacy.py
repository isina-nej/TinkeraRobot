...
def is_retrievable(item: MemoryItem, user_id: int, conversation_id: int | None) -> bool:
    ...
    if item.expires_at and item.expires_at <= timezone.now() and item.retention_level != MemoryItem.Retention.CRITICAL:
        return False
    if item.visibility in {MemoryItem.Visibility.PRIVATE, MemoryItem.Visibility.SYSTEM_ONLY}:
    ...
    if item.memory_scope == MemoryItem.Scope.USER:
        return item.visibility == MemoryItem.Visibility.USER_GLOBAL
    return item.conversation_id == conversation_id
